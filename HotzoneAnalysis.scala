...
val resultHotZone = spark.sql("select rectangle,count(point) as count from joinResult group by rectangle order by rectangle asc")
    resultHotZone.createOrReplaceTempView("HotZone")
    

    return resultHotZone // YOU NEED TO CHANGE THIS PART
 // YOU NEED TO CHANGE THIS PART
  }
