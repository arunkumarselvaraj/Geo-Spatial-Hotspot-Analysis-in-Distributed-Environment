...
def runHotcellAnalysis(spark: SparkSession, pointPath: String): DataFrame =
{
  ...
  val meanVal =  pickupInfo.agg(sum("hotness")).first.getLong(0).*(1.0)./(numCells)
  val standardDeviationVal = scala.math.sqrt(pickupInfo.withColumn("hotness2", pow("hotness", 2)).agg(sum("hotness2")).first.getDouble(0)./(numCells).-(meanVal.*(meanVal)))
  val pickupInfoMap = pickupInfo.collect().map(row => (row.getString(0),row.getLong(1))).toMap
  spark.udf.register("gScore",(cellVal: String)=> HotcellUtils.gScoreCalculate(cellVal, numCells.toInt, pickupInfoMap, minX.toInt, maxX.toInt, minY.toInt, maxY.toInt, minZ.toInt,   maxZ.toInt, meanVal, standardDeviationVal ))
